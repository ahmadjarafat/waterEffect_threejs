    ...
    for (let i = 0; i < 3; i++) {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          uZoomDelta: new THREE.Uniform(1),
      ...
      this.sceneManager.scene_1.add(mesh);
    }
  }
  onMouseMove(ev) {
...
vec2 withRatio(vec2 uv, vec2 canvasSize, vec2 textureSize){
    ...
    vec2 ratio = vec2(
        min((canvasSize.x*4. / canvasSize.y) / (textureSize.x / textureSize.y), 1.0),
        min((canvasSize.y*4. / canvasSize.x) / (textureSize.y / textureSize.x), 1.0)
      );

    return vec2(
        uv.x * ratio.x + (1.0 - ratio.x) ,
